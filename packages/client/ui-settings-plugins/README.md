...
- **Exposure is a per-namespace opt-in, not a Host allowlist** — a namespace is remotely configurable only when its owner registers it with `expose: true`, so a plugin distributed outside this repository surfaces its own configuration here by opting in; a namespace that never registered, or that registered without the opt-in, answers `settings-not-exposed`.
