...
Setting the default writes the `agent-presets` settings namespace, which its owner opts in to configuration clients with `expose: true` ([`dsh-apiproxy`](../../host/apiproxy/README.md) serves only namespaces that opted in — a namespace without the opt-in makes a picker move and then silently forget).
